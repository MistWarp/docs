...
MistWarp connects to a shared cloud server (`wss://clouddata.turbowarp.org`) by default, the same one TurboWarp uses. There is no limit on how many cloud variables a project may create (Scratch caps it at 10).
